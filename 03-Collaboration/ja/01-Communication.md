...
本文は `../../02-Governance/ja/01-Organization.md` のロール権限定義、`03-RFC-Process.md` の正式意思決定プロセス、`../../01-Foundation/ja/02-Security-Ethics.md` のセキュリティ開示とインシデント処理ルールに代わるものではありません。
...
flowchart TB
    subgraph FACT["事実源"]
        ...
        NOTION["Notion / Wiki<br/>組織ナレッジ / Owner Registry / Runbook / Onboarding"]
        GUIDE["Governance Repo<br/>原則 / ガバナンス / 協業 / エンジニアリング標準"]
    end
    subgraph CHANNEL["コミュニケーションとリズム"]
        FEISHU["飞书<br/>内部リアルタイムチャット / 会議 / カレンダー / ボット"]
        WX["微信群<br/>中文コミュニティ入口"]
        DC["Discord<br/>国際コミュニティ入口"]
        CAL["Calendar<br/>会議リズム / レビューウィンドウ / リリースウィンドウ"]
    end
    subgraph AUTOMATION["自動化"]
        BOT["Agent / Bot Layer<br/>ルーティング / 要約 / リマインダー / 状態チェック"]
    end
    FEISHU -.タスク発生.-> GH
    WX -.価値ある結論を書き戻し.-> GH
    DC -.価値ある結論を書き戻し.-> GH
...
flowchart LR
    ...
    REL --> PROJ["GitHub Project + 飞书リマインダー"]
    RES --> RLOG["Research Log"]
    DAILY --> CHAT["飞书"]
    MEET --> MIN["議事録 + Issue/RFC/ADR"]
    INC --> ICH["飞书 Incident Channel"]
    VULN --> SEC["非公開セキュリティチャネル"]
    ANN --> ANR["飞书公告 + GitHub/Notion 記録"]
...
```mermaid
flowchart TD
    START["会議が必要？"] --> A{"非同期で解決可能？"}
    A -- "はい" --> ASYNC["Issue / PR / RFC / Notion に変換"]
    A -- "いいえ" --> B{"アジェンダ、DRI、期待アウトプットあり？"}
    B -- "いいえ" --> NO["会議を作成しない"]
    B -- "はい" --> INVITE["カレンダーイベント作成<br/>目的 / アジェンダ / 事前読み / 関連リンク"]
    INVITE --> MEET["会議進行"]
    MEET --> DEC["Decisions を記録"]
    MEET --> ACT["Action Items を記録"]
    MEET --> RISK["Risks / Blockers を記録"]
    DEC --> WRITE["議事録に書き込む"]
    ACT --> ISSUE["GitHub Issue / Project に変換"]
    RISK --> ESC["必要時にエスカレーション"]
    WRITE --> LINK["Issue / RFC / ADR / Runbook をリンク"]
    ISSUE --> FOLLOW["会議後追跡"]
    ESC --> FOLLOW
```
...
2. 公開群で鍵、token、本番設定、ユーザープライバシー、未公開セキュリティ脆弱性、未許可素材を送ってはなりません。セキュリティ脆弱性は `../../01-Foundation/ja/02-Security-Ethics.md` §11 の非公開開示チャネルに従い、コミュニティ群では議論しません。
3. コミュニティ群の行動規範は `../../02-Governance/ja/02-Community.md` §9 が適用されます。ハラスメント、侮辱、差別、なりすまし、プライバシー漏洩、悪意のあるスパム、未許可の宣伝も禁止されます。
...
6. コミュニティ群は発表チャネルではありません——ロードマップ、セキュリティ声明、提携関係、法的約束、公告は、許可された者が公式身份でのみ発表できます（`../../02-Governance/ja/02-Community.md` §14 参照）。
...
flowchart LR
    ...
    CAL --> HUB
    NOTION --> HUB
    HUB --> RELAY["Kaguya Relay<br/>イベントルーティング"]
    HUB --> SCRIBE["Kaguya Scribe<br/>議事録草案"]
    HUB --> STEWARD["Kaguya Steward<br/>ガバナンスと引き継ぎリマインダー"]
    HUB --> SENTINEL["Kaguya Sentinel<br/>セキュリティとインシデントコミュニケーション"]
    RELAY --> FEISHU["飞书カード / 群リマインダー"]
    SCRIBE --> NOTE["Notion / GitHub 草案"]
    STEWARD --> DIGEST["Weekly Digest"]
    SENTINEL --> INCIDENT["Incident Channel / Timeline"]
    FEISHU -.事実源としてはならない.-> GH
    NOTE -.人間 DRI 確認後.-> NOTION["Notion / Wiki"]
    DIGEST -.リンクバック.-> GH
    INCIDENT -.事後蓄積.-> POST["Postmortem"]
```
...
本文は公開 RFC による改訂のみ可能です。旧版はバージョン管理に保存され、いつでも参照できます。`../../01-Foundation/ja/01-Principles.md` の「衝突と改訂」と一致：本文が組織権限、RFC プロセス、セキュリティルールと衝突する場合、対応する専門ドキュメントが優先します。法務、セキュリティ倫理の最低限の基準と衝突する場合、その基準が優先します。
