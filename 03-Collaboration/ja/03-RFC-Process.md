...
flowchart LR
    A["Author<br/>執筆と応答"]
    C["Champion<br/>推進と調整"]
    S["Sponsor<br/>プロセス进入を支援"]
    E["RFC Editor<br/>番号、形式、状態、索引"]
    RR["Required Reviewers<br/>専門レビュー"]
    D["Decision Owner<br/>最終裁定"]
    A --> C
    S --> A
    C --> E
    E --> RR
...
stateDiagram-v2
    [*] --> Idea
    Idea --> PreRFC: RFC 必要の可能性
    PreRFC --> Draft: 草案形成
    Draft --> Review: Editor チェック通過
    Review --> FCP: 主要問題処理済み
    FCP --> Accepted: 受理
    FCP --> Rejected: 拒否
    FCP --> Deferred: 延期
    Draft --> Withdrawn: 作者が撤回
    Review --> NeedsRevision: 修正が必要
    NeedsRevision --> Draft
    Accepted --> Active: 実装追跡を作成
    Active --> Implemented: 着地・検証
    Implemented --> Final
    Final --> Superseded: 新 RFC に置換
    Superseded --> Obsolete
    Rejected --> [*]
    Deferred --> PreRFC: 再評価
    Withdrawn --> [*]
...
flowchart TD
    D["十分な議論"] --> O["実質的異議を提起"]
    O --> P{"異議は具体的、関連、理解可能か？"}
    P -- "いいえ" --> NOTE["記録するがブロックしない"]
    P -- "はい" --> HANDLE["異議を真剣に処理"]
    HANDLE --> RES{"解決したか？"}
    RES -- "解決" --> FCP["FCP に進む"]
    RES -- "未解決" --> RECORD["未解決分岐と理由を記録"]
    RECORD --> FCP
    FCP --> DEC["Decision Owner が裁定"]
    DEC --> R{"裁定を受け入れるか？"}
    R -- "はい" --> END["結果をアーカイブ"]
    R -- "いいえ" --> APPEAL["組織エスカレーションパスで上訴"]
```
...
```mermaid
flowchart TB
    RFC["Accepted RFC<br/>方向が受理"] --> ADR{"アーキテクチャ決定を形成する？"}
    ADR -- "はい" --> A["ADR 作成 / 更新"]
    ADR -- "いいえ" --> N["ADR 不要、RFC のみ保持"]
    RFC --> SPEC{"標準 / 規範を変更する？"}
    SPEC -- "はい" --> STD["standards / API / AI / Embodiment ドキュメント更新"]
    SPEC -- "いいえ" --> SKIP["標準を更新しない"]
    RFC --> IMPL["Implementation Issue 作成"]
    IMPL --> PLAN["Planning Breakdown"]
    PLAN --> WORK["Engineering Workflow"]
    WORK --> QA["Quality Gates"]
    QA --> RELEASE["Release / Deploy"]
    RELEASE --> FINAL["RFC 状態を Implemented / Final に更新"]
```
