...
```mermaid
flowchart TB
    L0["QA-L0<br/>Draft<br/>草稿 / 非正式笔记<br/>标注状态"]
    L1["QA-L1<br/>Experiment<br/>实验 / Spike<br/>复现记录 / 数据来源 / 过期时间"]
    L2["QA-L2<br/>Maintained Component<br/>库 / CLI / SDK<br/>CI / 单元测试 / 文档 / Owner"]
    L3["QA-L3<br/>Production Service<br/>服务 / 数据管线 / API<br/>契约测试 / SLO / 监控 / Runbook / 回滚"]
    L4["QA-L4<br/>AI / Agent System<br/>Agent / RAG / 模型服务<br/>Eval / 对抗测试 / 模型卡 / 权限审查"]
    L5["QA-L5<br/>Embodied / Safety-Critical<br/>具身控制 / 执行器<br/>仿真 / HIL / E-Stop / HITL / 独立安全审查"]
    L0 --> L1 --> L2 --> L3 --> L4 --> L5
```
...
```mermaid
flowchart TB
    Q0["Gate Q0<br/>Quality Planning<br/>等级、风险、证据、回滚、Owner"]
    Q1["Gate Q1<br/>Design Quality<br/>不变量、失败模式、兼容、观测、回滚"]
    Q2["Gate Q2<br/>Implementation Quality<br/>PR、测试、错误处理、依赖、文档"]
    Q3["Gate Q3<br/>CI Quality Gate<br/>format / lint / typecheck / tests / scans"]
    Q4["Gate Q4<br/>Release Quality Gate<br/>制品、SBOM、监控、Runbook、回滚"]
    Q5["Gate Q5<br/>Operational Quality Gate<br/>SLO、告警、日志、Tracing、事故闭环"]
    Q0 --> Q1 --> Q2 --> Q3 --> Q4 --> Q5
    Q3 -.失败.-> BLOCK["阻断合并"]
    Q4 -.失败.-> HOLD["阻断发布"]
    Q5 -.异常.-> INCIDENT["Incident / Postmortem / Regression Test"]
    INCIDENT -.改进.-> Q0
```
...
flowchart TB
    ...
    UNIT["Unit Tests<br/>纯函数 / 领域逻辑 / 状态转移"]
    COMP["Component Tests<br/>模块边界 / UI 组件 / Tool Wrapper"]
    ...
    E2E["E2E Tests<br/>关键用户旅程 / 发布前 Smoke"]
...
```mermaid
flowchart TB
    CHANGE["AI / Agent 变更"] --> VER["版本固定<br/>模型 / Prompt / Policy / Tool"]
    VER --> DATA["数据与评测集检查<br/>来源 / 许可 / 污染 / 版本"]
    DATA --> PERM["工具权限矩阵"]
    PERM --> EVAL["能力评测"]
    EVAL --> ADV["对抗测试<br/>Prompt Injection / 越权 / 记忆污染"]
    ADV --> REG["行为回归测试"]
    REG --> COST["延迟 / 成本 / 循环预算"]
    COST --> LOG["审计日志完整性"]
    LOG --> DEC{"通过？"}
    DEC -- "是" --> RELEASE["进入发布 / 灰度"]
    DEC -- "否" --> FIX["修复 / 降级 / 禁用 / 回滚"]
```
...
flowchart TB
    ...
    DESIGN -.定义.-> LIMIT["速度 / 力 / 空间 / 姿态 / 工具边界"]
    SIM -.验证.-> SCENARIO["场景覆盖与失败模式"]
    HIL -.验证.-> SENSOR["传感器 / 执行器故障"]
    CTRL -.验证.-> ESTOP["E-Stop / Kill Switch"]
    HITL -.验证.-> OVERRIDE["人工接管路径"]
    ACCEPT -.要求.-> RUNBOOK["Runbook / Incident Path / 测试日志"]
```
...
```mermaid
flowchart TB
    R["Report<br/>报告"] --> T["Triage<br/>分类"]
    T --> REP["Reproduce<br/>复现"]
    REP --> C["Classify<br/>分级"]
    C --> RCA["Root Cause<br/>根因"]
    RCA --> FIX["Fix<br/>修复"]
    FIX --> REG["Regression Test<br/>防回归"]
    REG --> REL["Release<br/>发布"]
    REL --> V["Verify<br/>验证"]
    V --> CLOSE["Close<br/>关闭"]
    RCA --> DOC["文档 / Runbook 更新"]
    RCA --> MON["监控 / 告警改进"]
    RCA --> ADR["必要时 ADR / RFC"]
    CLOSE --> LEARN["沉淀质量债 / 改进项"]
```
