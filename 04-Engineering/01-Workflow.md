...
```mermaid
flowchart TD
    X["工程事项"] --> R{"是否涉及公共契约、生产、AI、数据、具身、安全、隐私、不可回滚迁移？"}
    R -- "是" --> H["High-risk Engineering Path"]
    R -- "否" --> S{"是否为新功能、可复用组件、服务、数据模块、需要部署或文档？"}
    S -- "是" --> STD["Standard Engineering Path"]
    S -- "否" --> L["Lightweight Path"]
...
```mermaid
flowchart TD
    T["技术选择 / 新依赖 / 新框架"] --> Q{"是否影响长期维护、核心依赖、部署、许可证、供应链或跨仓库契约？"}
    Q -- "否" --> ISSUE["在 Issue / PR 中记录理由"]
    Q -- "是" --> DEC["Technical Decision Record / ADR"]
    DEC --> A["Adopt<br/>默认推荐标准"]
    DEC --> TR["Trial<br/>限定项目试用"]
    DEC --> AS["Assess<br/>调研，不进入正式依赖"]
    DEC --> H["Hold<br/>暂停引入"]
    A --> USE["新项目默认使用"]
    TR --> PILOT["试点 + Owner + 退出路径"]
...
flowchart TB
    ...
    SEC --> CHECK
    E2E --> CHECK
    CHECK -- "否" --> FIX["修复 / 更新测试 / 重新提交"]
    FIX --> PR
    CHECK -- "是" --> REVIEW["Code Review / CODEOWNERS / Required Review"]
